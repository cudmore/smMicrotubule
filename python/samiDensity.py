...
def myDensity(path, genotype, sex, myCellNumber, doNapari=False):
	...
	resultDict = bimpy.util.getTiffFileInfo(path)
...
if __name__ == '__main__':
	...
	for batchFile in batchFileList:
		...
		for fileNum, file in enumerate(fileList):
			...
			args=(file, genotype, sex, myCellNumber)
			oneAnswer = pool.apply_async(myDensity, args=args)
	...
	for myCellNumber, result in enumerate(results):		
		tiffFileInfo = result.get() ### get results from parallel
		if tiffFileInfo is not None:
			fileDictList.append(tiffFileInfo)

	print('done processing numFiles:', numFiles)
	analysisRoot
	#saveDensityResultsFile = '/Users/cudmore/Desktop/density-results-xxx.csv'
	saveDensityResultsFile = os.path.join(analysisRoot, 'density-results.csv')
